...
def decrypt(cipherText):
    ...
    for i in range(len(cipherText)):
        ...
        #print(''.join(partial_cipher_list))

        for x in range(8):
            for y in range(8):
                partial_cipher_string += partial_cipher[IP[x][y]-1]
        #print(partial_cipher_string)
...
C = '11001100000000001100110011111111'
partial_msg_list = ["{0:01x}".format(int(C[4*j:4*j+4], 2)) for j in range(8)]
partial_msg = list(''.join(partial_msg_list))
partial_msg_string = ''
print(''.join(partial_msg_list))
